...
def AttachMuonSF(muons, year):
    ...
    #NEW years
    if year not in ['2016','2016APV','2017','2018','2022', '2022EE', '2023', '2023BPix']: raise Exception(f"Error: Unknown year \"{year}\".")
...
#    muons['sf_lo_3l_muon']  = (new_sf - new_err) * (reco_sf - reco_err) * (loose_sf - loose_err) * (iso_sf - iso_err)
    muons['sf_nom_2l_muon'] = ak.ones_like(new_sf)
    muons['sf_hi_2l_muon'] = ak.ones_like(new_sf)
    muons['sf_lo_2l_muon'] = ak.ones_like(new_sf)
    muons['sf_nom_3l_muon'] = ak.ones_like(new_sf)
    muons['sf_hi_3l_muon'] = ak.ones_like(new_sf)
    muons['sf_lo_3l_muon'] = ak.ones_like(new_sf)
...
def AttachElectronSF(electrons, year):
    ...
    pt = electrons.pt
    #NEW years
    if year not in ['2016','2016APV','2017','2018','2022','2022EE','2023','2023BPix']:
...
#    electrons['sf_lo_3l_elec']  = (reco_do) * (new_sf_3l - new_err_3l) * (loose_sf - loose_err) * (iso_sf - iso_err)
    electrons['sf_nom_2l_elec'] = ak.ones_like(reco_sf)
    electrons['sf_hi_2l_elec'] = ak.ones_like(reco_sf)
    electrons['sf_lo_2l_elec'] = ak.ones_like(reco_sf)
    electrons['sf_nom_3l_elec'] = ak.ones_like(reco_sf)
    electrons['sf_hi_3l_elec'] = ak.ones_like(reco_sf)
    electrons['sf_lo_3l_elec'] = ak.ones_like(reco_sf)
