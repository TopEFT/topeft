...
class run3leptonselection:
    ...
    def coneptElec(self, ele):
        conePt = (0.90 * ele.pt * (1 + ele.jetRelIso))
        return ak.where((ele.mvaTTH>get_te_param("mva_TTH_e_cut_run3")),ele.pt,conePt)

    def coneptMuon(self, muo):
        conePt = (0.90 * muo.pt * (1 + muo.jetRelIso))
        return ak.where(((muo.mvaTTH>get_te_param("mva_TTH_m_cut_run3"))&(muo.mediumId>0)),muo.pt,conePt)
    ...
    def isFOElec(self, ele, year):
        ...
        ptReq      = (ele.conept>get_te_param("fo_pt_cut"))
        qualityReq = (ele.idEmu & ele.convVeto & (ele.lostHits==0))
        mvaReq     = ((ele.mvaTTH>get_te_param("mva_TTH_e_cut_run3")) | ((ele.mvaIso > get_te_param("fo_e_mvaiso_cut_run3"))  & (ele.jetRelIso < get_te_param("fo_e_jetRelIso_cut"))))

    def isFOMuon(self, muo, year):
        bTagCut=get_medium_btag_foryear(year)
        btagReq = (muo.btagDeepFlavB<bTagCut)
        ptReq   = (muo.conept>get_te_param("fo_pt_cut"))
        mvaReq  = ((muo.mvaTTH>get_te_param("mva_TTH_m_cut_run3")) | ((muo.btagDeepFlavB<smoothBFlav(0.9*muo.pt*(1+muo.jetRelIso),20,45,year)) & (muo.jetRelIso < get_te_param("fo_m_jetRelIso_cut")) & (muo.sip3d < smoothSip3D(0.9*muo.pt*(1+muo.jetRelIso),2.5,8.,15,45))))
        return ptReq & btagReq & mvaReq

    def tightSelElec(self, ele):
        return (ele.isFO) & (ele.mvaTTH > get_te_param("mva_TTH_e_cut_run3"))

    def tightSelMuon(self, muo):
        return (muo.isFO) & (muo.mediumId>0) & (muo.mvaTTH > get_te_param("mva_TTH_m_cut_run3"))
