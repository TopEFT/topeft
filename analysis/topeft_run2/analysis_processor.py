...
from topeft.modules.corrections import GetBTagSF, ApplyJetCorrections, GetBtagEff, AttachMuonSF, AttachElectronSF, AttachPerLeptonFR, GetPUSF, ApplyRochesterCorrections, ApplyJetSystematics, AttachPSWeights, AttachScaleWeights, GetTriggerSF, AttachTauSF, ApplyTES, ApplyTESSystematic
...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def __init__(self, samples, wc_names_lst=[], hist_lst=None, ecut_threshold=None, do_errors=False, do_systematics=False, split_by_lepton_flavor=False, skip_signal_regions=False, skip_control_regions=False, muonSyst='nominal', dtype=np.float32):
        ...
        self._accumulator = processor.dict_accumulator({
            ...
            #"b0pt"    : HistEFT("Events", wc_names_lst, hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("systematic", "Systematic Uncertainty"),hist.Cat("appl", "AR/SR"), hist.Bin("b0pt",    "Leading b jet  $p_{T}$ (GeV)", 25, 0, 500)),
            ...
            #"ht"      : HistEFT("Events", wc_names_lst, hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("systematic", "Systematic Uncertainty"),hist.Cat("appl", "AR/SR"), hist.Bin("ht",      "H$_{T}$ (GeV)", 100, 0, 2000)),
            ...
            #"tauJetBtag": HistEFT("Events", wc_names_lst, hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("systematic", "Systematic Uncertainty"),hist.Cat("appl", "AR/SR"), hist.Bin("tauJetBtag",   "flav of jet cleaned by tau", 10, 0, 10)),
        })
    ...
    def process(self, events):
        ...
        ngenevents         = self._samples[dataset]["nGenEvents"]
        isWJets = histAxisName.startswith("WJetsToLNu")
        ...
        #print("before selections vsjet:", ak.to_list(tau.idDeepTau2017v2p1VSjet))
        #print("before selections vse:", ak.to_list(tau.idDeepTau2017v2p1VSe))
        #print("before selections vsmu:", ak.to_list(tau.idDeepTau2017v2p1VSmu))
        #print("before selections pt:", ak.to_list(tau.pt))
        #print("before selections eta:", ak.to_list(abs(tau.eta)))
        ...
        #print("default=",ak.fill_none(tau.cleanmask == 1, True))
        ...
        tau["isTight"]   = te_os.isTightTau(tau.idDeepTau2017v2p1VSjet)
        tau["isVTight"]  = te_os.isVTightTau(tau.idDeepTau2017v2p1VSjet)
        ...
        ##cleaning_taus = tau[tau["isMedium"]>0]
        #tau = tau[tau['DMflag']]
        ...
        #print("after selections:", ak.to_list(tau.idDeepTau2017v2p1VSjet))
        #print(ak.to_list(tau["isMedium"]))
        #print("after selections pt:", ak.to_list(tau.pt))
        #print("\n\n\n\n\n\n\n\n\n")
        ...
        tau_to_clean = ak.with_name(tau, "something")
        tau_tmp = ak.cartesian([ak.local_index(jets.pt), tau_to_clean.jetIdx], nested=True)
        tau_jets = jets[ak.any(tau_tmp.slot0 == tau_tmp.slot1, axis=-1)] 
        nTauJets = ak.num(tau_jets)
        tau_jets = ak.pad_none(tau_jets, 1)

        #print("jets matched with taus:", ak.to_list(jets.pt))
        #print("jets indices:", ak.to_list(ak.local_index(jets.pt)))
        #print("tau_jet_cartesian:", ak.to_list(tau_tmp))
        #print("matching_mask:", ak.to_list(ak.any(tau_tmp.slot0 == tau_tmp.slot1, axis=-1)))
        #print("jets matched with taus:", ak.to_list(tau_jets.pt))
        #print("\n\n\n\n\n\n\n\n")

        near, tau["dR"] = tau.nearest(l_fo, return_metric=True)
        ...
        obj_correction_syst_lst = [
            ...
            f'TES_{year}Up', f'TES_{year}Down',
            #f'FES_{year}Up', f'FES_{year}Down',
        ]
        ...
        if not isData:
            ...
            if eft_coeffs is None: 
                genws = events["genWeight"]
            else: 
                genws = np.ones_like(events["event"])
            
            if isWJets:
                ratio = sow / ngenevents
                genw = ak.full_like(events["genWeight"], ratio)
            else:    
                genw = genws
        ...
        for syst_var in syst_var_list:
            ...
            if not isData:
                ...
                #### Applying TES and FES systematics
                tau["pt"], tau["mass"]      = ApplyTESSystematic(year, tau, syst_var)
            ...
            tauJetBtag = (tau_jets[:,0].btagDeepFlavB > btagwpl)
            if not isData:
                GenJet  = events.GenJet
                gnbjets = ak.num(GenJet[np.abs(GenJet.partonFlavour)==5])
            ...
            for ch_name in ["2l", "2los", "2l_4t", "3l", "4l", "2l_CR", "2l_CRflip", "3l_CR", "2los_CRtt", "2los_CRZ", "1l_1tau_CR", "1l_CRZ"]:
                ...
                # For data only
                if isData:
                    if ch_name in ["2l","2los","2l_4t","2l_CR","2l_CRflip"]:
            ...
            if not isData:
                tau_jetFake = ((tau0.genPartFlav < 1) | (tau0.genPartFlav > 5))
                tau_lepFake = ((tau0.genPartFlav > 0) & (tau0.genPartFlav < 5))
                tau_genuine = (tau0.genPartFlav == 5)
            ...
            # b jet masks
            bmask_atleast1med_or_atleast2loose = ((nbtagsm>=1)|(nbtagsl>=2))
            ...
            selections.add("2lss_p", (events.is2l & chargel0_p & bmask_atleast1med_or_atleast2loose & pass_trg & bmask_atmost2med & no_tau_mask))  # Note: The ss requirement has NOT yet been made at this point! We take care of it later with the appl axis
            selections.add("2lss_m", (events.is2l & chargel0_m & bmask_atleast1med_or_atleast2loose & pass_trg & bmask_atmost2med & no_tau_mask))  # Note: The ss requirement has NOT yet been made at this point! We take care of it later with the appl axis

            # 2lss selection (enriched in 4 top)
            selections.add("2lss_4t_p", (events.is2l & chargel0_p & bmask_atleast1med_or_atleast2loose & pass_trg & bmask_atleast3med))  # Note: The ss requirement has NOT yet been made at this point! We take care of it later with the appl axis
            selections.add("2lss_4t_m", (events.is2l & chargel0_m & bmask_atleast1med_or_atleast2loose & pass_trg & bmask_atleast3med))  # Note: The ss requirement has NOT yet been made at this point! We take care of it later with the appl axis
            
            #2l + 1tau selection
            selections.add("2lss_p_1tau", (events.is2l & chargel0_p & bmask_atleast1med_or_atleast2loose & pass_trg & bmask_atmost2med & tau_L_mask))
            selections.add("2lss_m_1tau", (events.is2l & chargel0_m & bmask_atleast1med_or_atleast2loose & pass_trg & bmask_atmost2med & tau_L_mask))
            selections.add("2los_onZ_1tau", (events.is2l & charge2l_0 & sfosz_2l_mask & bmask_atleast2med & pass_trg & tau_L_mask))
            selections.add("2los_onZ_2tau", (events.is2l & charge2l_0 & sfosz_2l_mask & bmask_atleast2med & pass_trg & tau_2L_mask))
            selections.add("2lss_2tau", (events.is2l & bmask_atleast1med_or_atleast2loose & pass_trg & tau_2L_mask))
            ...
            # 2los selection
            selections.add("2los_CRtt_Ftau", (events.is2l_nozeeveto & charge2l_0 & bmask_atleast1med_or_atleast2loose & pass_trg & tau_Fake_mask)) 
            selections.add("2los_CRtt_Ttau", (events.is2l_nozeeveto & charge2l_0 & bmask_atleast1med_or_atleast2loose & pass_trg & tau_L_mask))
            selections.add("2los_CRZ", (events.is2l_nozeeveto & charge2l_0 & sfosz_2l_mask & bmask_exactly0med & pass_trg & no_tau_mask))
            selections.add("2los_1tau_CRZ", (events.is2l_nozeeveto & charge2l_0 & sfosz_2l_mask & pass_trg & tau_L_mask))

            selections.add("1l_1tau_os_CR", (events.is1l & tau_L_mask & pass_trg & weight_mask & chargelt_0 & bmask_atleast1med_or_atleast2loose))
            selections.add("1l_1tau_CR", (events.is1l & tau_L_mask & pass_trg & weight_mask & bmask_atleast1med_or_atleast2loose))
            selections.add("1l_CR", (events.is1l & ~tau_L_mask & pass_trg & weight_mask & bmask_atleast1med_or_atleast2loose))
            ...
            selections.add("3l_m_offZ_1b", (events.is3l & charge3l_m & ~sfosz_3l_mask & bmask_exactly1med & pass_trg & no_tau_mask))
            selections.add("3l_p_offZ_2b", (events.is3l & charge3l_p & ~sfosz_3l_mask & bmask_atleast2med & pass_trg & no_tau_mask))
            selections.add("3l_m_offZ_2b", (events.is3l & charge3l_m & ~sfosz_3l_mask & bmask_atleast2med & pass_trg & no_tau_mask))
            selections.add("3l_onZ_1b", (events.is3l & sfosz_3l_mask & bmask_exactly1med & pass_trg & no_tau_mask))
            selections.add("3l_onZ_2b", (events.is3l & sfosz_3l_mask & bmask_atleast2med & pass_trg & no_tau_mask))
            selections.add("3l_1tau_1b", (events.is3l & bmask_exactly1med & pass_trg & tau_L_mask))
            selections.add("3l_1tau_2b", (events.is3l & bmask_exactly2med & pass_trg & tau_L_mask))
            ...
            #if not isData:
            #    selections.add("tau_jetFake", tau_jetFake)
            #    selections.add("tau_lepFake", tau_lepFake)
            #    selections.add("tau_genuine", tau_genuine)
            ...
            sr_cat_dict = {
                "2l" : {
                    "exactly_4j" : {
                        "lep_chan_lst" : ["2lss_p" , "2lss_m", "2lss_4t_p", "2lss_4t_m", "2lss_p_1tau", "2lss_m_1tau", "2lss_2tau"],
                ...
                "2los" : { #new!
                    #"exactly_4j" : {
                    #    "lep_chan_lst" : ["2los_onZ_1tau", "2los_onZ_2tau"],
                    #    "lep_flav_lst" : ["ee" , "em" , "mm"],
            ...
            cr_cat_dict = {
                ...
                "1l_1tau_CR": {
                    "exactly_0j": {
                        "lep_chan_lst" : ["1l_1tau_CR", "1l_1tau_os_CR", "1l_CR"],
                        "lep_flav_lst" : ["e", "m"],
                        "appl_lst"     : ["isSR_1l", "isAR_1l"]
                     },
                    "exactly_1j": {
            ...
            tau_cons = ["tau_lepFake", "tau_jetFake", "tau_genuine", "blank"]
                            ...
                            for appl in cat_dict[nlep_cat][njet_val]["appl_lst"]:
                                ...
                                for lep_chan in cat_dict[nlep_cat][njet_val]["lep_chan_lst"]:
                                    ...
                                    for lep_flav in cat_dict[nlep_cat][njet_val]["lep_flav_lst"]:
                                        ...
                                        if ((("tau" in dense_axis_name) or ("dR" in dense_axis_name)) and ("tau" not in ch_name)): continue
                                        ...
                                        if (("all" in ch_name) and ("genw" not in dense_axis_name)): continue
                                        if (("tauJetBtag" in dense_axis_name) and ("1l_1tau_jet_CR" not in ch_name)): continue

                                        #if (not isData) and ("blank" not in con):
                                        #    temp_histAxisName = histAxisName
                                        #    histAxisName = histAxisName + "_" + con
                                        #    cuts_lst.append(con)
