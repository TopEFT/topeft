...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        genEle = ele[genEle_mask]
        genMu_mask = np.abs(mu.matched_gen.pdgId)==13
        genMu = mu[genMu_mask]
        genleps = ak.concatenate([genEle,genMu],axis=1)
        genlep_pt_mask = ak.fill_none(ak.pad_none(genleps.pt > 20, 1),False)         #Require that the lepton pt be at least 20 GeV.
        genleps = genleps[genlep_pt_mask]
        ...
        genjet = genJet[genJet.pt > 30]          #Require that the jet pt be at least 30 GeV.
        ...
        genphoton_pt = ak.fill_none(ak.firsts(genPhoton.pt),-1)
