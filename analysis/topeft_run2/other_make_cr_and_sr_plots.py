...
SR_CHAN_DICT = {
    ...
    #],
    "2lss_offZ_1tau_SR": [
        "2lss_offZ_m_1tau_3j", "2lss_offZ_m_1tau_4j", "2lss_offZ_m_1tau_5j", "2lss_offZ_m_1tau_6j", "2lss_offZ_m_1tau_7j",
        "2lss_offZ_p_1tau_3j", "2lss_offZ_p_1tau_4j", "2lss_offZ_p_1tau_5j", "2lss_offZ_p_1tau_6j", "2lss_offZ_p_1tau_7j",
    ],
    "2lss_onZ_1tau_SR": [
        "2lss_onZ_m_1tau_3j", "2lss_onZ_m_1tau_4j", "2lss_onZ_m_1tau_5j", "2lss_onZ_m_1tau_6j", "2lss_onZ_m_1tau_7j",
        "2lss_onZ_p_1tau_3j", "2lss_onZ_p_1tau_4j", "2lss_onZ_p_1tau_5j", "2lss_onZ_p_1tau_6j", "2lss_onZ_p_1tau_7j",
    ],
    #"2lss_2tau_SR": [
    #    "2lss_2tau_3j", "2lss_2tau_4j", "2lss_2tau_5j", "2lss_2tau_6j", "2lss_2tau_7j",
    #],
    #"2los_onZ_1tau_SR": [
    #    "2los_onZ_1tau_4j","2los_onZ_1tau_5j", "2los_onZ_1tau_6j", "2los_onZ_1tau_7j",
    #],
    #"2los_1b_1tau_SR": [
    #    "2los_1b_1tau_4j", "2los_1b_1tau_5j", "2los_1b_1tau_6j", "2los_1b_1tau_7j",
    #],
    #"2los_offZ_1tau_SR": [
    #    "2los_offZ_1tau_4j","2los_offZ_1tau_5j", "2los_offZ_1tau_6j", "2los_offZ_1tau_7j",
    #],
    #"2los_2tau_SR": [
    #    "2los_onZ_2tau_3j", "2los_onZ_2tau_4j", "2los_onZ_2tau_5j", "2los_onZ_2tau_6j", "2los_onZ_2tau_7j",
    #],
    #"3l_offZ_SR" : [
    ...
    #],
    #"3l_1tau_SR": [
    #    "3l_1tau_1b_2j", "3l_1tau_1b_3j", "3l_1tau_1b_4j", "3l_1tau_1b_5j",
    #    "3l_1tau_2b_2j", "3l_1tau_2b_3j", "3l_1tau_2b_4j", "3l_1tau_2b_5j",
    #],
    #"4l_SR" : [
    #    "4l_2j", "4l_3j", "4l_4j",
    #]
...
def make_all_sr_plots(dict_of_hists,year,unit_norm_bool,save_dir_path,split_by_chan=True,split_by_proc=False):
    #print(dict_of_hists["taupt"])
    wcs = { "cpt": 10.10, "ctp": 9.72, "cptb": 5.68, "cQlMi": 2.47, "cQq81": 0.59, "cQq11": 0.32, "cQl3i": 3.11, "ctq8": 0.56, "ctlTi": 0.44, "ctq1": 0.33, "ctli": 2.35, "cQq13": 0.14, "cbW": 1.48, 
            "cpQM": 11.82, "cpQ3": 2.39, "ctei": 2.46, "cQei": 2.28, "ctW": 0.84, "ctlSi": 3.0, "cQq83": 0.31, "ctZ": 1.09, "ctG": 0.37, "ctt1": 1.17, "cQt1": 1.85, "cQt8": 4.11, "cQQ1": 2.38}
    # If selecting a year, append that year to the wight list
    #sig_wl = ["private"]
    if year is None: pass
    elif year == "2017": sig_wl.append("UL17")
    ...
    #    if "data" in proc_name:
    #        samples_to_rm_from_sig_hist.append(proc_name)
    ...
    for proc_name in all_samples:
        if "data" in proc_name:
            ...
            SR_MAP["ttH"].append(proc_name)
        elif ("ttlnu" in proc_name):
            SR_MAP["ttlnu"].append(proc_name)
        elif ("ttll" in proc_name):
            SR_MAP["ttll"].append(proc_name)
        elif ("tllq" in proc_name) or ("tHq" in proc_name):
            SR_MAP["tXq"].append(proc_name)
        elif ("tttt" in proc_name):
            SR_MAP["tttt"].append(proc_name)
        #elif ("DY" in proc_name):
        #    SR_MAP["DY"].append(proc_name)
        #elif("TT" in proc_name):
        #    SR_MAP["ttbar"].append(proc_name)
        else:
            SR_MAP["Bkg"].append(proc_name)
    #sig_sample_lst = utils.filter_lst_of_strs(all_samples,substr_whitelist=sig_wl)
    #if len(sig_sample_lst) == 0: raise Exception("Error: No signal samples to plot.")
    #samples_to_rm_from_sig_hist = []
    #for sample_name in all_samples:
    ...
    #        samples_to_rm_from_sig_hist.append(sample_name)
    print("\nAll samples:",all_samples)
    print("\nRM samples:",samples_to_rm_from_sig_hist)


    # Loop over hists and make plots
    skip_lst = ["ptz", "ptz_wtau", "ht", "met", "l1eta", "l1pt", "ptbl"] # Skip this hist
    for idx,var_name in enumerate(dict_of_hists.keys()):
        #if yt.is_split_by_lepflav(dict_of_hists): raise Exception("Not set up to plot lep flav for SR, though could probably do it without too much work")
        if (var_name in skip_lst): continue
        ...
        print("HERE!!!")
        print(dict_of_hists[var_name].identifiers("channel"))
        hist_sig = dict_of_hists[var_name].remove(samples_to_rm_from_sig_hist,"sample")
        hist_sig = hist_sig.integrate("systematic","nominal")
        hist_sig.set_wilson_coefficients(**wcs)
        ...
        if split_by_chan:
            for hist_cat in SR_CHAN_DICT.keys():
                ...
                if (("tau" in var_name) and ("tau" not in hist_cat)): continue
                ...
                hist_sig_integrated_ch = group_bins(hist_sig_integrated_ch,SR_MAP)
                #hist_sig_integrated_ch = group_bins(hist_sig_integrated_ch,CR_GRP_MAP)
                #signal_vals = hist_sig_integrated_ch.integrate("sample", "Signal").values()[()]
                #bkg_vals = hist_sig_integrated_ch.integrate("sample", "Bkg").values()[()]
                #fom = 0
                #for (sig_val, bkg_val) in zip(signal_vals, bkg_vals):
                #    if sig_val <= 0 or bkg_val <= 0:
                #        continue
                #    fom += (sig_val)/(math.sqrt(sig_val+bkg_val))*(sig_val)/(math.sqrt(sig_val+bkg_val))
                #print("FOM!!!")
                #print(sr_cat_dict[hist_cat])
                #print(hist_cat)
                #print(math.sqrt(fom))
                #print("")
                #big_fom += fom
                #if ("tau" not in hist_cat):
                #    big_fom_not += fom
                # Make the plots
                print(sr_cat_dict[hist_cat])
        ...
        print("BIG FOM!!!")
        print(math.sqrt(big_fom))
        print(math.sqrt(big_fom_not))
        # Make plots for each process
        if split_by_proc:
            for proc_name in sig_sample_lst:
