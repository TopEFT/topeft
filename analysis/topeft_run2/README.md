...
* `run_analysis.py`:
    ...
    - The data-driven helper now supports inline and deferred workflows. Keep the historical behaviour by relying on the default `--np-postprocess=inline` (paired with `--do-np`) so the `_np.pkl.gz` file appears immediately. Choose `--np-postprocess=defer` to emit the base pickle along with `histos/<outname>_np.pkl.gz.metadata.json`, which records the resolved years, follow-up command, and absolute histogram paths for later processing. Setting `--np-postprocess=skip` suppresses the data-driven step entirely.
