...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def __init__(self, samples, wc_names_lst=[], hist_lst=None, ecut_threshold=None, do_errors=False, do_systematics=False, split_by_lepton_flavor=False, skip_signal_regions=False, skip_control_regions=False, muonSyst='nominal', dtype=np.float32, rebin=False):
        ...
        histograms = {}
        pt_axis = hist.axis.Variable(axes_info["photon_pt_eta"]["pt"]["variable"],name='photon_pt',label=r"$p_{T}$ $\gamma$ (GeV)")
        pt_sumw2_axis = hist.axis.Variable(axes_info["photon_pt_eta"]["pt"]["variable"],name='photon_pt_sumw2',label=r"$p_{T}$ $\gamma$ (GeV)")
        abseta_axis = hist.axis.Variable(axes_info["photon_pt_eta"]["abseta"]["variable"],name='photon_abseta',label=r"Photon abs. $\eta$")
    ...
    def process(self, events):
        ...
        #Selection of fakeablePhoton, and photons in regC and regD are done in objection selection script
        ph_fo = cleanPh[cleanPh.fakeablePhoton_MR]          #For determination of FR, we need to relax the medium cut on the photons.

        ph_fo['inC_ABCD'] = ph_fo.mediumPhoton_regC
        ph_fo['inD_ABCD'] = ph_fo.mediumPhoton_regD
        ...
        for syst_var in syst_var_list:
            ...
            #Note: JEC not applied on data. We had discussed this in the past, but we did not reach a particular conclusion.
            if not isData:
                cleanedJets["pt_raw"] = (1 - cleanedJets.rawFactor)*cleanedJets.pt
                cleanedJets["mass_raw"] = (1 - cleanedJets.rawFactor)*cleanedJets.mass
            ...
            te_es.addPhotonSF(events)
            te_es.categorizePhotonsInABCD_FR(events, sampleType) #we want prompt photons by default
            ...
            if not isData:
                ...
                #Revisit: Once we have the "good" ttgamma samples, we won't need to apply these correction factors
            ...
            lep_cats = ["2los_np_sf","2los_np_of"]
            ...
            # AR/SR categories
            selections.add("isMR_2lOS_C_ABCD",    ( events.is2l_SR) & charge2l_0 & events.isC_FR_ABCD)   #the isC_FR_ABCD mask also has prompt photon mask baked in for MC
            selections.add("isMR_2lOS_D_ABCD",    ( events.is2l_SR) & charge2l_0 & events.isD_FR_ABCD)   #the isD_FR_ABCD mask also has prompt photon mask baked in for MC

            selections.add("2los_sf",(retainedbyOverlap & events.is2l_nozeeveto & (events.is_ee | events.is_mm) & ~sfosz_2los_ll_mask & bmask_atleast0med & charge2l_0 & pass_trg & ~sfosz_2los_llg_mask_medph & exactly_1ph))
            selections.add("2los_of",(retainedbyOverlap & events.is2l & charge2l_0 & events.is_em & bmask_atleast0med & pass_trg & exactly_1ph))
            ...
            cat_dict = {
                "2los_np_sf" : {
                    "atleast_1j" : {
                        ...
                        "lep_flav_lst" : ["ee", "mm"],
                        "appl_lst"     : ["isMR_2lOS_C_ABCD","isMR_2lOS_D_ABCD"],
                    },
                },
                "2los_np_of" : {
                    "atleast_0j" : {
                                ...
                                for lep_chan in cat_dict[nlep_cat][njet_val]["lep_chan_lst"]:
                                    ...
                                    for lep_flav in cat_dict[nlep_cat][njet_val]["lep_flav_lst"]:
                                        ...
                                        if dense_axis_name == "photon_pt_eta":
                                            ...
                                            plot_help.fill_2d_histogram(hout, dense_axis_name, "photon_pt", "photon_abseta", photon_pt, photon_abseta, ch_name, appl, histAxisName, wgt_fluct, weight_2DHist, None, all_cuts_mask, suffix="")  #absolutely make sure to set eft_coeffs to None
                                            plot_help.fill_2d_histogram(hout, dense_axis_name, "photon_pt", "photon_abseta", photon_pt, photon_abseta, ch_name, appl, histAxisName, wgt_fluct, np.square(weight_2DHist), None, all_cuts_mask, suffix="_sumw2") #absolutely make sure to set eft_coeffs to None
