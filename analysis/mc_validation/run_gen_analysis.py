...
if __name__ == '__main__':
    ...
    if args.hist_list == ["ana"]:
        ...
        hist_lst = ["cosZ", "cosq", "azim", "tops_pt", "tX_pt", "njets"]
    ...
    for sname in samplesdict.keys():
        ...
        samplesdict[sname]['post_mortem'] = samplesdict[sname]['post_mortem'] if 'post_mortem' in samplesdict[sname] else False
        ...
        print('   - post_mortem  : %s'   %samplesdict[sname]['post_mortem'])
        if not samplesdict[sname]["isData"]:
            for wgt_var in WGT_VAR_LST:
    ...
    if len(wc_lst) == 0:
        for k in samplesdict.keys():
            if samplesdict[k]['post_mortem']:
                for wc in samplesdict[k]['PMWCnames']:
                    if wc not in wc_lst:
                        wc_lst.append(wc)
            else:
    ...
    processor_instance = gen_processor.AnalysisProcessor(samplesdict,wc_lst,hist_lst,ecut_threshold,do_errors,do_systs,split_lep_flavor,skip_sr,skip_cr)
