...
DATA_ERR_OPS = {'linestyle':'none', 'color':'k'}
...
def remove(histo, bins, axis_name):
    ...
    """
    if axis_name not in (x.name for x in histo.axes):
        raise ValueError(f"{axis_name} is not a axis of the histogram.")
    axis = histo.axes[axis_name]
    keep = [bin for bin in axis if bin not in bins]
    index = [axis.index(bin) for bin in keep]

    full_slice = tuple(slice(None) if ax != axis else index for ax in histo.axes)
    return histo[full_slice]
...
def group_bins(histo,bin_map,axis_name="process",drop_unspecified=False):
    ...
    new_histo = group(histo, old_ax.name,old_ax.name,bin_map)
...
def get_rate_syst_arrs(base_histo,proc_group_map):
    ...
    for rate_sys_type in grs.get_syst_lst():
        ...
        for sample_name in yt.get_cat_lables(base_histo,"process"):
            ...
            thissample_nom_arr = base_histo.integrate("process",sample_name).integrate("systematic","nominal").values()[()]
...
def get_shape_syst_arrs(base_histo):
    ...
    for syst_name in syst_var_lst:
        ...
        n_arr = base_histo.integrate("process",relevant_samples_lst).integrate("systematic","nominal").values()[()] # Sum of all samples for nominal variation
        ...
        # If the syst is not renorm or fact, just treat it normally (correlate across all processes)
            ...
            u_arr_sum = base_histo.integrate("process",relevant_samples_lst).integrate("systematic",syst_name+"Up").values()[()]   # Sum of all samples for up variation
            d_arr_sum = base_histo.integrate("process",relevant_samples_lst).integrate("systematic",syst_name+"Down").values()[()] # Sum of all samples for down variation
    ...
    for proc_grp in grp_map.keys():
        ...
        if proc_grp == "Signal":
            for proc_name in proc_lst:
                ...
                n_arr_proc = base_histo.integrate("process",proc_name).integrate("systematic","nominal").values()[()]
                u_arr_proc = base_histo.integrate("process",proc_name).integrate("systematic",syst_name+"Up").values()[()]
                d_arr_proc = base_histo.integrate("process",proc_name).integrate("systematic",syst_name+"Down").values()[()]
        ...
        # Otherwise corrleated across groups (e.g. ZZ and WZ, as datacard maker does in SR)
            ...
            n_arr_grp = base_histo.integrate("process",proc_lst).integrate("systematic","nominal").values()[()]
            u_arr_grp = base_histo.integrate("process",proc_lst).integrate("systematic",syst_name+"Up").values()[()]
            d_arr_grp = base_histo.integrate("process",proc_lst).integrate("systematic",syst_name+"Down").values()[()]
...
def make_cr_fig(h_mc,h_data,unit_norm_bool,set_x_lim=None,err_p=None,err_m=None,err_ratio_p=None,err_ratio_m=None):
    ...
    # Plot the MC
    hists = [h_mc[{'process': proc}] for proc in h_mc.axes['process']]
...
def make_all_sr_sys_plots(dict_of_hists,year,save_dir_path):
    ...
    for idx,var_name in enumerate(dict_of_hists.keys()):
        ...
        hist_sig = remove(dict_of_hists[var_name],samples_to_rm_from_sig_hist,"process")
        #hist_sig = dict_of_hists[var_name].remove(samples_to_rm_from_sig_hist,"process")
...
def make_all_sr_data_mc_plots(dict_of_hists,year,save_dir_path):
    ...
    for proc_name in mc_sample_lst + data_sample_lst:
        ...
        elif ("TTZToLL_M1to10" in proc_name):
            SR_GRP_MAP["ttll"].append(proc_name)
    ...
    #analysis_bins = {
    #    'njets': {
    #        '2l': [4,5,6,7,dict_of_hists['njets'].axes['njets'].edges()[-1]], # Last bin in topeft.py is 10, this should grab the overflow
    #        '3l': [2,3,4,5,dict_of_hists['njets'].axes['njets'].edges()[-1]],
    #        '4l': [2,3,4,dict_of_hists['njets'].axes['njets'].edges()[-1]]
    #    }
    #}
    #analysis_bins['ptz'] = [0, 200, 300, 400, 500, dict_of_hists['ptz'].axes['ptz'].edges()[-1]]
    #analysis_bins['lj0pt'] = [0, 150, 250, 500, dict_of_hists['lj0pt'].axes['lj0pt'].edges()[-1]]
    analysis_bins = {}
    for key in axes_info:
        if 'variable' in axes_info[key]:
            analysis_bins[key] = axes_info[key]['variable']
    #analysis_bins = {k:v['variable'] for k,v in axes_info.items()}
    ...
    for idx,var_name in enumerate(dict_of_hists.keys()):
        ...
        hist_mc_orig = remove(dict_of_hists[var_name],samples_to_rm_from_mc_hist,"process")
        #hist_mc_orig = dict_of_hists[var_name].remove(samples_to_rm_from_mc_hist,"process")
        hist_data_orig = remove(dict_of_hists[var_name],samples_to_rm_from_data_hist,"process")
        #hist_data_orig = dict_of_hists[var_name].remove(samples_to_rm_from_data_hist,"process")

        # Loop over channels
        channels_lst = list(dict_of_hists[var_name].axes["channel"])
        #channels_lst = yt.get_cat_lables(dict_of_hists[var_name],"channel")
        ...
        for chan_name in SR_CHAN_DICT.keys():
            ...
            hist_mc = hist_mc_orig.integrate("channel",SR_CHAN_DICT[chan_name])
            hist_data = hist_data_orig.integrate("channel",SR_CHAN_DICT[chan_name])
            #hist_mc = hist_mc_orig.integrate("systematic","nominal").integrate("channel",SR_CHAN_DICT[chan_name],flow="all")
            #hist_data = hist_data_orig.integrate("systematic","nominal").integrate("channel",SR_CHAN_DICT[chan_name],flow="all")

            print('Grouping', {k:v for k,v in SR_GRP_MAP.items() if 'Data' not in k and len(v)},"process")
            hist_mc = group_bins(hist_mc,{k:v for k,v in SR_GRP_MAP.items() if 'Data' not in k and len(v)},"process")
            hist_data = group_bins(hist_data,{k:v for k,v in SR_GRP_MAP.items() if 'Data' in k and len(v)},"process")[{'process': sum}]
...
def make_all_cr_plots(dict_of_hists,year,skip_syst_errs,unit_norm_bool,save_dir_path):
    ...
    for idx,var_name in enumerate(dict_of_hists.keys()):
        ...
        # Extract the MC and data hists
        hist_mc = remove(dict_of_hists[var_name],samples_to_rm_from_mc_hist,"process")
        #hist_mc = dict_of_hists[var_name].remove(samples_to_rm_from_mc_hist,"process")
        hist_data = remove(dict_of_hists[var_name],samples_to_rm_from_data_hist,"process")
        ...
        for hist_cat in cr_cat_dict.keys():
            if (hist_cat == "cr_2los_Z" and (("j0" in var_name) and ("njets" not in var_name))): continue # The 2los Z category does not require jets (so leading jet plots do not make sense)
            if (hist_cat == "cr_2lss_flip" and (("j0" in var_name) and ("njets" not in var_name))): continue # The flip category does not require jets (so leading jet plots do not make sense)
            print("\n\tCategory:",hist_cat)
            ...
            hist_mc_integrated = remove(hist_mc_integrated,samples_to_rm,"process")
            #hist_mc_integrated = hist_mc_integrated.remove(samples_to_rm,"process")
            ...
            if not skip_syst_errs:
                ...
                if (var_name == "njets"):
                    ...
                    db_hist = hist_mc_integrated.integrate("process",CR_GRP_MAP["Diboson"]).integrate("systematic","nominal").values()[()]
                ...
                nom_arr_all = hist_mc_integrated.sum("process").integrate("systematic","nominal").values()[()]
